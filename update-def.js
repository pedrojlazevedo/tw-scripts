(function () {
    ...
    // Aviso na consola se faltar configuração (mas não pára a execução para o botão poder aparecer)
    ...
    function getCurrentGroupName() {
        ...
        if (popupSelect.length) return popupSelect.text().trim();
        ...
        return new Promise((resolve, reject) => {
            ...
            const interval = setInterval(() => {
                ...
                if (Date.now() - start > timeoutMs) {
                    clearInterval(interval);
                    reject(new Error('Timeout: a popup do contador não apareceu.'));
                }
            }, 500);
        });
    }
    ...
    function scrapeAndInjectDiscord() {
        ...
        try {
            ...
            console.error('Erro ao re-injetar:', err);
        }
    }

    function hookChangeGroup() {
        if (STATE.hooked || typeof window.villagesTroopsCounter === 'undefined') return;
    ...
    function startPeriodicCheck() {
        ...
        STATE.checkInterval = setInterval(() => {
            ...
            if (table && !discordBtn) scrapeAndInjectDiscord();
        }, 1500);
    }
        ...
        const html = `
            <div class="discord-button-row" style="display:flex;flex-direction:column;align-items:center;gap:10px;margin:14px auto;">
                <div style="background:#f9f3e0;border:1px solid #d6c58a;border-radius:8px;padding:10px 14px;font-size:11px;color:#4a3513;width:90%;max-width:360px;">
                    <b>Jogador:</b> ${playerName}<br>
                    <b>Grupo:</b> ${currentGroup || '<i>nenhum</i>'}<br>
                    ${!CONFIG.webhookUrl ? '<br><b style="color:#c0392b;">⚠️ Erro: Webhook não configurado no Tampermonkey! O envio vai falhar.</b>' : ''}
                </div>
                <button id="sendDefesa" type="button"
                    style="padding:12px 28px;cursor:pointer;background:linear-gradient(to bottom,#f2e5b6,#d6c58a);border:1px solid #b59e4c;border-radius:10px;font-size:15px;font-weight:bold;color:#4a3513;">
                    Partilhar Defesa
                </button>
                <div id="defesaReward" style="display:none;text-align:center;">
                    <div style="font-size:13px;font-weight:bold;color:#27ae60;margin-bottom:8px;">Enviado com sucesso!</div>
                    <img id="defesaRewardImg" src="" style="max-width:200px;border-radius:10px;">
                </div>
            </div>
        `;

        const popup = $('#popup_box_import .popup_box_content');
        if (popup.length) popup.append(html);

        $('#sendDefesa').on('click', function () {
            if (!CONFIG.webhookUrl) {
                alert("Por favor, adiciona o teu Webhook no script Loader do Tampermonkey!");
                return;
            }
            sendToDiscord(troops);
            
            // Só envia para as sheets se a variável tiver sido definida
    ...
    function sendToDiscord(troops) {
        ...
        var fields = [
            { name: 'Tribo', value: tribeName || '-', inline: true },
            { name: 'Grupo', value: currentGroup || '-', inline: true }
        ];
        
        ALL_UNITS.forEach(u => {
            if(troops[u]) fields.push({ name: u, value: String(troops[u]), inline: true });
        });
        ...
        window.jQuery.ajax({
            ...
            data: JSON.stringify({
                content: `**Defesa Atualizada: ${playerName}**`,
                embeds: [{ title: 'Tropas Disponíveis', fields: fields, color: 3066993 }]
            })
        });
    }
    ...
    function sendToGoogleSheets(troops) {
        var payload = {
            playerName: window.game_data.player.name,
            groupName: getCurrentGroupName(),
            tribeName: window.game_data.player.ally || '',
            troops: troops
        };
        fetch(CONFIG.googleSheetsUrl, { method: 'POST', mode: 'no-cors', body: JSON.stringify(payload) });
    }
    ...
    function injectStyles() {
        ...
        style.textContent = `
            #tmDefesaLauncher {
                ...
                padding: 14px 20px; background: #f6e8bc; border: 1px solid #8b6a2c; border-radius: 14px;
                color: #4a3513; font-weight: bold; cursor: pointer;
            }
            #tmDefesaLauncher.stale { border-color: #c0392b; animation: defesaPulse 2.5s infinite; }
            @keyframes defesaPulse { 50% { box-shadow: 0 0 18px 4px rgba(192,57,43,0.45); } }
        `;
    ...
    function renderLauncherButton() {
        ...
        btn.innerHTML = '🛡️ <span id="tmDefesaLabel">Abrir Defesa</span><br><small id="tmDefesaStatus"></small>';
        btn.addEventListener('click', async () => {
            ...
            try {
                ...
                } else { window.villagesTroopsCounter.init(); }
                ...
                console.error("Erro a abrir contador:", e);
    ...
    function updateLauncherStatus() {
        ...
        const isStale = !lastUpdated || (Date.now() - lastUpdated) >= STALE_THRESHOLD_MS;
        status.textContent = getStaleMessage(lastUpdated, Date.now());
